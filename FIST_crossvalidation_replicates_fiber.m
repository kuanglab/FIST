...
lambdavec = [0.01,0.1,1];
for nameid = 1:3
    cd(work_path); 
    % prepare data tensor and graphs
    [V,W,rank_k] = data_prep_Rep(nameid, data_path, utils_path);
    opts.rank = rank_k;
    ...
    for fold = 1:5
        ...
        name=[res_path,'FIST_replicates_fiber_',data_bag{nameid},'_fold',num2str(fold),'.mat'];
        save(name,'maeres','maperes','r2res','lambdavec','-v7.3'); 
    end
end
